...
import type { InferenceLog } from "@/lib/agents";

export function RecentInferences({ entries }: { entries: InferenceLog[] }) {
  return (
    <div className="panel">
      ...
      {entries.length === 0 ? (
        <div className="px-4 py-6 text-sm text-text-muted">no inference activity yet</div>
      ) : (
        <ul>
          {entries.map((e) => (
            <li
