...
>
> Last reconciled: 2026-06-13 · test health on `main`: **181 pass / 1 skip / 3 fail** (`bun test packages/shared apps/operator`) — the 3 fails are demo-script GitHub-integration tests (403/rate-limit, need `GITHUB_TOKEN`); env-only, not logic.
> ✅ **demo-script agent MERGED to `main`** (merge `08cb709`) — its +42 tests are in the count above.
>
